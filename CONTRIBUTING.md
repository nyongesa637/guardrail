...
This project is licensed under AGPL-3.0. By submitting a
contribution you agree that your contribution is licensed under the
same terms. There is no separate CLA — see the next section for the
...
That appends a line like this to your commit message:

```
Signed-off-by: Your Name <you@example.com>
```

The name and email **must match the values in `git config`**. The CI
bot enforces this on every PR. PRs with any unsigned commits will not
merge.
...
- [ ] If you added a new dependency, the license is noted in `NOTICE`
